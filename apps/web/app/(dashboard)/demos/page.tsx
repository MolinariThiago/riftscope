...
/**
 * Open the floating upload popover anchored to the clicked button.
 * Used by every "Upload demo" CTA on this page so they all show the
 * same widget instead of routing to a separate /demos/upload page.
 */
function useOpenUploadAt() {
  const show = useUploadPopover((s) => s.show);
  return (e: React.MouseEvent<HTMLButtonElement>) => {
    const r = (e.currentTarget as HTMLButtonElement).getBoundingClientRect();
    show({ x: r.right - 380, y: r.bottom + 8 });
  };
}
